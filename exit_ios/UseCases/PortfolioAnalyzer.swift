...
enum PortfolioAnalyzer {
    ...
    static func calculateRegionAllocation(
        ...
        var usWeight: Double = 0
        var krWeight: Double = 0
        ...
        for holding in holdings {
            ...
            if stock.info.exchange.isUS {
                usWeight += holding.weight
            } else {
                krWeight += holding.weight
            }
        }
        
        var result: [RegionAllocation] = []
        
        if usWeight > 0 {
            result.append(RegionAllocation(region: "미국", flag: "🇺🇸", weight: usWeight))
        }
        if krWeight > 0 {
            result.append(RegionAllocation(region: "한국", flag: "🇰🇷", weight: krWeight))
        }
